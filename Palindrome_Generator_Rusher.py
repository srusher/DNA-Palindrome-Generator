...
def palindrome_generator():
    ...
    l = len(dna_list) # determining the length of the sequence
    ...
    ## Both of the following 'elif' statements will start in the middle of the array and work their way out to create a palindrome
        ...
        i = 0 # recursive value used in for loop below
        ...
        for base in range(1,(round(l/2))): #this loop will iterate across the dna_list equal to half of the lenght of the list
        ...
        if len(mirror_array) <= len(palindrome_array): # determines if the length of the palindrome array (that was just created) is longer than the mirror palindrome (generated at the beginning of the program). Whichever one is the shortest length will be printed below.
            print(f"\nThe palindrome with the fewest inserted bases for the given sequence is:\n\n{mirror_string}\n\nThe length of the original input sequnece was: {len(dna)}\nThe length of the new palindrome is: {len(mirror_string)}\n")
        ...
        if len(mirror_array) <= len(palindrome_array): # same process as the first 'elif' statement
            print(f"\nThe palindrome with the fewest inserted bases for the given sequence is:\n\n{mirror_string}\n\nThe length of the original input sequnece was: {len(dna)}\nThe length of the new palindrome is: {len(mirror_string)}\n")
